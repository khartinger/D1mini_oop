//_____D1_class_GSMmodule.h___________________181230-181230_____
...
class GSMmodule {
 ...
 public:
  GSMmodule();
  GSMmodule(bool swSerial);            // hard-/software serial
  GSMmodule(bool swSerial, String mem);// hard-/soft-serial+mem
  GSMmodule(String mem);               // sws+mem + default pins
  GSMmodule(int pinRx, int pinTx);     // sws+pins + default mem
  GSMmodule(int pinRx, int pinTx, String mem); // sws+pins+mem
  ~GSMmodule();
  void setup();

//_____D1_class_GSMmodule.cpp_________________181230-181230_____
...
GSMmodule::GSMmodule() { setup(true,-1,-1,GSMMOD_SMS_MEM); }

//_____constructor: set software/hardware serial________________
GSMmodule::GSMmodule(bool swSerial) 
{ setup(swSerial, -1,-1,GSMMOD_SMS_MEM); }

//_____constructor: set software/hardware serial and memory_____
GSMmodule::GSMmodule(bool swSerial, String mem) 
{ setup(swSerial, -1,-1,mem); }
  
//_____constructor: default pins, specified memory for sms______
GSMmodule::GSMmodule(String mem) { setup(true, -1,-1, mem); }
 
//_____constructor: specified pins, default memory for sms______
GSMmodule::GSMmodule(int pinRx, int pinTx) 
{ setup(true, pinRx, pinTx, GSMMOD_SMS_MEM); } 
 
//_____constructor: specified pins, memory for sms______________
GSMmodule::GSMmodule(int pinRx, int pinTx, String mem)
...
//_____setup GSMmodule__________________________________________
void GSMmodule::setup() { setup(true,-1,-1,GSMMOD_SMS_MEM); }

//_____setup GSMmodule__________________________________________
void GSMmodule::setup(bool swSerial,int pinRx, int pinTx, String mem)
{
 SWSerial_=swSerial;
 status=GSMMOD_OK;
...
int GSMmodule::getStatus() { return status; }

//_____Status as little result text (e.g. for OLED display)_____
String GSMmodule::getsStatus() {
 switch(status) {
...
int GSMmodule::setNumOfSms1(int num1) {
 if(num1>0) { 
  numOfSms1_=num1;
...
int GSMmodule::getNumOfSms1() { return numOfSms1_; }
...
int GSMmodule::sendSMS(SMS sms1)
{
...
bool GSMmodule::isSMS1() { return isSMS(numOfSms1_); }

//_____read first SMS in used memory____________________________
SMS GSMmodule::readSMS1() { return(readSMS(numOfSms1_)); }

//_____delete first SMS in used memory__________________________
// return: ERROR code
int GSMmodule::deleteSMS1() { return(deleteSMS(numOfSms1_)); }
...
// return: ERROR code
int GSMmodule::deleteSMS(int num)
{
...
int GSMmodule::sendNetwork(String net) {
...
// return: status
int GSMmodule::requestBalanceSMS() {
 SMS sms1;
...
String GSMmodule::extractBalance(String smstext) {
 String sRet="";
 int pos1, pos2;
...
String GSMmodule::sendCmd(String cmd) { 
 return(sendCmd(cmd, 2, GSMMOD_CEND, GSMMOD_WAIT_MS));
}

//_____send a standard command to SIM800C_______________________
String GSMmodule::sendCmd(String cmd, int numEnd) { 
 return(sendCmd(cmd, numEnd, GSMMOD_CEND, GSMMOD_WAIT_MS));
}

//_____send a standard command to SIM800C_______________________
String GSMmodule::sendCmd(String cmd, int numEnd, char cEnd) { 
...
String GSMmodule::sendCmd(String cmd, int numEnd, char cEnd, unsigned long ms)
{
...
bool GSMmodule::isSoftwareSerial() { return SWSerial_; }

//_____set software (true) or hardware (false) serial for module
// return: status after methode begin()
int GSMmodule::setSoftwareSerial(bool swSerial) {
 SWSerial_=swSerial;
...
String GSMmodule::hexStringToString(String sHex)
{
 String sRet="";
...
int GSMmodule::count_1in2(String sFind, String sBasic)
{
 int iPos=0;
 int iNum=0;

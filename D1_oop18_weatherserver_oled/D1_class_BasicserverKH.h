...
#define DEFAULT_SIGN       "WS"             //2 sign bytes EEPROM
...
#define DEFAULT_IP         192,168,1,120    // device ip
...
class BasicserverKH : public ESP8266WebServer {
 ...
 protected:
  ...
  IPAddress lastClient;                //last client connected
 ...
 public:
  ...
  IPAddress getLastClient() { return lastClient; }
...
//_____is a client now connected?_______________________________
bool BasicserverKH::isClient()
{
 if(!isConnected()) return false;
 IPAddress tempIP=client().remoteIP();
 if(tempIP[0]==0) return false;
 lastClient=tempIP;
 return true;
}
...
bool BasicserverKH::handleClientReconnect()
{
 ...
 if(!connect_) 
 {
  if(DEBUG1) Serial.println("handleClientReconnect(): No WLAN");
 }
 else
 {
  //-----check for client is connected now (isClient)-----------
  IPAddress tempIP=getLastClient();
  if(isClient()) {
   if(DEBUG1) {
    if(tempIP!=getLastClient())
    {
     String sIPclient=getLastClient().toString();
     Serial.println("handleClientReconnect()-lastClient: "+sIPclient);
    }
   } 
  }
...
//  Serial.println("check ok");
 ...
 if(DEBUG1)Serial.println("getParams: checksum="+String(tmp_[3]));

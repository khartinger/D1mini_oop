//_____D1_class_SoftSerial2.cpp_______________190101-190101_____
/*
SoftSerial2.cpp - Implementation of the Arduino software serial for ESP8266.
...
SoftSerial2::SoftSerial2(int receivePin, int transmitPin, bool inverse_logic, unsigned int buffSize) {
...
bool SoftSerial2::isValidGPIOpin(int pin) {
  return (pin >= 0 && pin <= 5) || (pin >= 12 && pin <= MAX_PIN);
}

void SoftSerial2::begin(long speed) {
...
long SoftSerial2::baudRate() {
  return F_CPU/m_bitTime;
}

void SoftSerial2::setTransmitEnablePin(int transmitEnablePin) {
...
void SoftSerial2::enableIntTx(bool on) {
  m_intTxEnabled = on;
}

void SoftSerial2::enableTx(bool on) {
...
void SoftSerial2::enableRx(bool on) {
  if (m_rxValid) {
    if (on)
...
size_t SoftSerial2::write(uint8_t b) {
...
void SoftSerial2::flush() {
  m_inPos = m_outPos = 0;
}

bool SoftSerial2::overflow() {
  bool res = m_overflow;
  m_overflow = false;
  return res;
}

int SoftSerial2::peek() {
  if (!m_rxValid || (m_inPos == m_outPos)) return -1;
  return m_buffer[m_outPos];
}

void ICACHE_RAM_ATTR SoftSerial2::rxRead() {
